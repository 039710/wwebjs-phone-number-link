    ...
    async initialize() {
        ...
        if (needAuthentication) {
            ...
            const handleLinkWithPhoneNumber = async () => {
                console.log('handleLinkWithPhoneNumber')
                const LINK_WITH_PHONE_BUTTON = 'span._3iLTh';
                const PHONE_NUMBER_INPUT = 'input.selectable-text';
                const NEXT_BUTTON = 'div>div>div[role="button"]._1M6AF';
                const CODE_CONTAINER = '[aria-details="link-device-phone-number-code-screen-instructions"]';
                const GENERATE_NEW_CODE_BUTTON = 'div[role="button"][tabindex="0"]._1M6AF';
            ...
            if (linkingMethod.isQR()) {
                console.log('Authentification with QR');
                await handleLinkWithQRCode();
            } else {
                console.log('Authentification with Phone Number');
